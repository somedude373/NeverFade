...
namespace Server.Items
{
	public class Trash4TokensBackpack : Container
	{
		...
		[Constructable]
		public Trash4TokensBackpack() : base( 0xE76 )
		{
			Name = "Trash 4 Neverfade Tokens Backpack"; 
			Movable = true;
			Hue = 1173;
        ...
        {
            if (dropped is NeverfadeTokens || dropped is TokenCheck)
            {
        ...
        {
            if (item is NeverfadeTokens || item is TokenCheck)
            {
                from.SendMessage("It would not be wise to trash tokens for tokens!");
                return false;
            }
