...
#if USE_TOKENS
			if (currency is NeverfadeTokens && TokenLedger)
				return AddTokensPile(currency as NeverfadeTokens);
...
#if USE_TOKENS
		public bool AddTokensPile(NeverfadeTokens tokens)
		{
			if ( !TokenLedger )
				return false;
...
#if USE_TOKENS
			List<Item> tokens;
			Type tokensType = intoTokensCheck? typeof(TokenCheck) : typeof(NeverfadeTokens);
...
#if USE_TOKENS
				if ( currencyType != typeof(NeverfadeTokens) )
#endif
			{
				if ( informPlayer )
...
#if USE_TOKENS
			else if ( currencyType == typeof(NeverfadeTokens) )
			{
				if (HasTokens(amount))
				{
		...
		public bool TryConsume(Type[] types, int[] amounts)
		{
			...
			for (int i = 0; i < types.Length && hasEnough; ++i)
			{
				...
				if ( currencyType == typeof(Gold) && HasGold(amounts[i]) ) //we have enough gold
					continue;
#if USE_TOKENS
				if ( currencyType == typeof(NeverfadeTokens) && HasTokens(amounts[i]) ) //we have enough tokens
					continue;
#endif
		...
		public bool TryStoreItem(Item item, int amount)
		{
			...
			foreach (Type type in types.Keys)
			{
				...
				if (type == typeof(Gold) && GoldAmount + usedAmount < GoldAmount)
					return false;
#if USE_TOKENS
				else if (type == typeof(NeverfadeTokens) && TokensAmount + usedAmount < TokensAmount)
					return false;
#endif
...
#if USE_TOKENS
				else if (type == typeof(NeverfadeTokens))
...
#if USE_TOKENS
				if (currencyType == typeof(NeverfadeTokens) && TokenLedger)
				{
					TokensAmount += (ulong)amount;
					return true;
				}
